...
# --- FUNCIONES DE PERSISTENCIA CON SESSION STATE Y DATOS POR DEFECTO ---
...
if st.session_state.pedido_seleccionado is not None:
    ...
    st.markdown(f"## 📂 Detalle del Pedido: #{row['ID_Base']:03d}-{row['ArchivoPC']}")
...
with tab_nuevo:
    with st.form("form_nuevo_pedido", clear_on_submit=True):
        ...
        if st.form_submit_button("Guardar Pedido", type="primary") and cliente:
            ahora = datetime.utcnow() - timedelta(hours=3)
            historial_inicial = [{"Fecha_Obj": ahora, "Fecha": ahora.strftime("%d/%m/%Y"), "Hora": ahora.strftime("%H:%M"), "Vendedor": st.session_state.vendedor_actual, "Concepto": "Seña", "Monto": sena, "Medio": mp_sena}] if senn > 0 else []
...
with tab_lista:
    if st.session_state.pedidos.empty:
        ...
        for idx, row in st.session_state.pedidos.sort_values(by="Fecha_Obj").iterrows():
            with st.container():
...
# --- RUTAS Y ARCHIVOS LOCALES (Para caché temporal en la Web) ---
# En Streamlit Cloud guardamos en una carpeta local temporal y luego la API lo subirá a Drive
RUTA_LOCAL = "datos_temporales"
os.makedirs(RUTA_LOCAL, exist_ok=True)

ARCHIVO_PEDIDOS = os.path.join(RUTA_LOCAL, "pedidos_taller.pkl")
ARCHIVO_GASTOS = os.path.join(RUTA_LOCAL, "gastos_taller.pkl")
ARCHIVO_USUARIOS = os.path.join(RUTA_LOCAL, "usuarios_taller.pkl")

# --- INTEGRACIÓN CON API DE GOOGLE DRIVE (Preparado para Streamlit Cloud) ---
def sincronizar_con_drive_api():
    """
    Esta función está preparada para conectarse con la API de Google Drive.
    En Streamlit Cloud, debes usar st.secrets para almacenar tu credentials.json
    y usar la librería google-api-python-client para subir los archivos .pkl
    """
    try:
        # Lógica futura de API:
        # 1. Autenticar usando st.secrets["gcp_service_account"]
        # 2. Buscar si el archivo pedidos_taller.pkl ya existe en tu Google Drive
        # 3. Subirlo/Sobrescribirlo usando media = MediaFileUpload(ARCHIVO_PEDIDOS)
        pass 
    except Exception as e:
        st.error(f"Error de conexión con Google Drive API: {e}")

# --- FUNCIONES DE GUARDADO Y CARGA ---
def guardar_datos():
    try:
        # 1. Guarda en el servidor temporal (o PC local)
        with open(ARCHIVO_PEDIDOS, "wb") as f: pickle.dump(st.session_state.pedidos, f)
        with open(ARCHIVO_GASTOS, "wb") as f: pickle.dump(st.session_state.gastos, f)
        with open(ARCHIVO_USUARIOS, "wb") as f: pickle.dump(st.session_state.usuarios, f)
        
        # 2. Llama a la API para enviar a la nube (Drive)
        sincronizar_con_drive_api()
        
        st.toast("¡Datos guardados y sincronizados!", icon="✅")
    except Exception as e:
        st.error(f"Error al guardar: {e}")

def cargar_datos():
    # Aquí en el futuro se descargan primero los archivos de Drive API antes de leerlos
    
    if os.path.exists(ARCHIVO_USUARIOS):
        with open(ARCHIVO_USUARIOS, "rb") as f: st.session_state.usuarios = pickle.load(f)
    else:
        st.session_state.usuarios = {"admin": "1234"}

    if os.path.exists(ARCHIVO_PEDIDOS):
        with open(ARCHIVO_PEDIDOS, "rb") as f: st.session_state.pedidos = pickle.load(f)
    else:
        st.session_state.pedidos = pd.DataFrame(columns=[
            "ID_Base", "Cliente", "Telefono", "Prenda", "Cantidad", "Diseno", "ArchivoPC", "TablaTalles", 
            "Observaciones", "Imagen", "Estado", "Vendedor", "Fecha_Obj", "Fecha", "Hora", "Anio",
    ...
    if os.path.exists(ARCHIVO_GASTOS):
        with open(ARCHIVO_GASTOS, "rb") as f: st.session_state.gastos = pickle.load(f)
    else:
        st.session_state.gastos = pd.DataFrame(columns=["Fecha_Obj", "Fecha", "Item", "Cantidad", "Precio_Unitario", "Total"])
...
if st.session_state.pedido_seleccionado is not None:
    ...
    with col_v3:
        # --- ELIMINAR PEDIDO (CON CONFIRMACIÓN) ---
        with st.popover("🗑️ Eliminar Pedido"):
            st.error("¿Estás seguro de que quieres eliminar este pedido de la base de datos de forma permanente?")
            if st.button("Sí, Confirmar Eliminación", type="primary", key="del_ped"):
                st.session_state.pedidos = st.session_state.pedidos.drop(idx)
                st.session_state.pedido_seleccionado = None
                guardar_datos()
